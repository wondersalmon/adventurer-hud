...
Поиск находит предметы по названию предмета или активности в бою, заклинаниях и
инвентаре. Звезда добавляет предмет или конкретную активность в избранное этого
пользователя для персонажа. Если у предмета несколько доступных активностей,
нажатие открывает выбор в HUD; Shift-нажатие использует нативное действие D&D 5e.
...
меню окна HUD. Часто используемые параметры доступны непосредственно в настройках
модулей Foundry. **Дополнительные настройки** содержат группы быстрого доступа,
использования предметов, управления окном и спасбросков от смерти. Поиск,
избранное и выбор активности включаются отдельно. Рядом находится сброс настроек.
Через меню окна можно открыть настройки, закрепить HUD, включить закрытие после
броска и сбросить размер и положение окна.

Настройки и избранное сохраняются для пользователя, а размер и положение окна —
...
`scripts/hud`. Там же находятся обработчики действий, логика окна и отдельные
модули боевых предметов, состояний и ресурсов. CSS-файлы из `styles` загружаются
в порядке, указанном в `module.json`. Диспетчеризация рендера находится в
`scripts/render`. Инструкция
